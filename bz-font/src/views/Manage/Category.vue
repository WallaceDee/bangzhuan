  ...
  <div>
    ...
    <Drawer :title="drawerTitle" v-model="drawerVisible" width="720">
      <Form :model="form" :label-width="120" ref="form" :rules="rule">
        <FormItem label="Category图片" prop="url">
          <Upload
            ref="upload"
...
export default {
            ...
            return [
              h(
                'Poptip',
                {
                  ...
                  on: {
                    'on-ok': () => {
                      this.setCategoryEnable(params.row)
                    }
                  }
                },
                [
  ...
  methods: {
    set2Top(Category) {
      setCategoryUpdateTime(Category).then(res => {
        if (res.status) {
          this.$Notice.success({
            title: '提示',
    ...
    doSumbit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          publicCategory(this.form).then(res => {
            if (res.status) {
